...
const SearchBox = {
	...
	computed: {
		suggestionsList() {
			/* eslint-disable */
			return Array.isArray(this.$props.suggestions) && this.$props.suggestions.length
				? withClickIds(this.$props.suggestions)
				: Array.isArray(this.$props.defaultSuggestions) &&
				  this.$props.defaultSuggestions.length
				? withClickIds(this.$props.defaultSuggestions)
				: [];
			/* eslint-enable */
		},
		topSuggestions() {
	...
	watch: {
		...
		dataField() {
			this.updateQueryOptions();
			this.triggerCustomQuery(this.$props.componentId, this.$data.currentValue, this.$props);
		},
		highlightField() {
			this.updateQueryOptions();
		},
		...
		defaultQuery(newVal, oldVal) {
			if (!isQueryIdentical(newVal, oldVal, this.$data.currentValue, this.$props)) {
				this.triggerDefaultQuery(this.$data.currentValue, this.$props);
			}
		},
		customQuery(newVal, oldVal) {
			if (!isQueryIdentical(newVal, oldVal, this.$data.currentValue, this.$props)) {
				this.triggerCustomQuery(this.componentId, this.$data.currentValue, this.$props);
			}
		},
		suggestions(newVal) {
	...
	methods: {
		handleText(value) {
			if (this.$props.autosuggest) {
				this.triggerDefaultQuery(value, this.$props);
			} else {
				this.triggerCustomQuery(this.$props.componentId, value, this.$props);
			}
		},
		validateDataField() {
				...
				if (isDefaultValue) {
					...
					if (props.strictSelection) {
						if (cause === causes.SUGGESTION_SELECT || value === '') {
							this.triggerCustomQuery(props.componentId, value, props);
						} else {
							this.setValue('', true);
						}
					} else {
		...
		triggerDefaultQuery(value, props) {
			let query = SearchBox.defaultQuery(value, props);
		...
		triggerCustomQuery(componentId, value, props) {
			const { customQuery, filterLabel, showFilter, URLParams } = props;

			const defaultQueryTobeSet = SearchBox.defaultQuery(value, props);
	...
	render() {
		const { theme, innerClass, expandSuggestionsContainer } = this.$props;
		const { recentSearchesIcon, popularSearchesIcon } = this.$scopedSlots;
		const hasSuggestions = this.suggestionsList.length;
