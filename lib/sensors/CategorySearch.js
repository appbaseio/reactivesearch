	...
	_createClass(CategorySearch, [{
		...
		value: function defaultSearchQuery(input) {
			...
			if (input && input.value) {
				var _ret = function () {
					var query = [];
					var appbaseField = _this3.fieldType === "string" ? [_this3.props.appbaseField] : _this3.props.appbaseField;
					...
					if (input.category && input.category !== null) {
						...
						return {
							v: {
								bool: {
									must: [query, {
										term: _defineProperty({}, _this3.props.categoryField, input.category)
									}]
								}
							}
						};
