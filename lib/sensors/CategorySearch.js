...
var CategorySearch = function (_Component) {
	...
	_createClass(CategorySearch, [{
		...
		value: function highlightQuery(props) {
			var fields = {};
			var highlightFields = props.highlightFields ? props.highlightFields : props.appbaseField;
		...
		value: function defaultSearchQuery(input) {
			...
			if (input && input.value) {
				var query = [];
				var appbaseFields = this.fieldType === "string" ? [this.props.appbaseField] : this.props.appbaseField;
			...
			if (loadSuggestions) {
				data.hits.hits.forEach(function (hit) {
					if (_this6.fieldType === "string") {
						var _searchField = hit._source[_this6.props.appbaseField].trim();
		...
		value: function render() {
			...
			return _react2.default.createElement(
				"div",
				{ className: "rbc rbc-categorysearch col s12 col-xs-12 card thumbnail " + cx + " " + (this.state.isLoadingOptions ? "is-loading" : ""), style: this.props.componentStyle },
				title,
