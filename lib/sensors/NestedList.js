	...
	_createClass(NestedList, [{
		...
		value: function nestedAggQuery() {
			...
			var createTermQuery = function createTermQuery(index) {
				return {
					term: _defineProperty({}, _this7.props.dataField[index], _this7.state.selectedValues[index])
				};
			};
		...
		value: function setValue(value) {
			...
			var execQuery = function execQuery() {
				...
				_reactivemaps.AppbaseSensorHelper.URLParams.update(_this11.props.componentId, paramValue, _this11.props.URLParams);
		...
		value: function renderItems(items) {
			...
			return items.map(function (item, index) {
				...
				var cx = (0, _classnames2.default)({
					"rbc-item-active": Array.isArray(_this12.state.selectedValues) && item.key === _this12.state.selectedValues[level],
					"rbc-item-inactive": !(Array.isArray(_this12.state.selectedValues) && item.key === _this12.state.selectedValues[level])
				});
				if (Array.isArray(_this12.state.selectedValues) && _this12.state.selectedValues.length) {
					if (item.key === _this12.state.selectedValues[level] || _this12.state.selectedValues.length === level) {
						return _react2.default.createElement(
							...
							_react2.default.createElement(
								...
								{ className: "rbc-list-item " + cx, onClick: function onClick() {
										return _this12.onItemClick(item.value.join("@rbc@"), level);
									} },
								_react2.default.createElement(
									"span",
									{ className: "rbc-label" },
							...
							Array.isArray(_this12.state.selectedValues) && _this12.state.selectedValues[level] === item.key && _this12.state.items[level + 1] ? _react2.default.createElement(
								"ul",
								{ className: "rbc-list-container col s12 col-xs-12" },
