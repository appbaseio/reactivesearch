	...
	_createClass(TagCloud, [{
		...
		value: function setValue(value) {
			...
			var execQuery = function execQuery() {
				...
				_this7.defaultSelected = _this7.selectedValue;
				_reactivemaps.AppbaseSensorHelper.URLParams.update(_this7.props.componentId, obj.value, _this7.props.URLParams);
				_reactivemaps.AppbaseSensorHelper.selectedSensor.set(obj, true);
			};
		...
		value: function renderTags() {
			...
			return this.state.items.map(function (item) {
				var size = item.doc_count / _this8.highestCount * (max - min) + min;
				...
				return _react2.default.createElement(
					"a",
					{
						...
						onClick: function onClick() {
							return _this8.setValue(item.key);
						},
						key: item.key,
					...
					item.key,
					" ",
					_this8.props.showCount ? _react2.default.createElement(
						"span",
						{ className: "rbc-count" },
						item.doc_count
					) : ""
