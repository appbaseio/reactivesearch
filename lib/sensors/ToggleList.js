		...
		value: function changeValue(defaultValue) {
			...
			if (!_.isEqual(this.defaultSelected, defaultValue)) {
				...
				if (!this.props.multiSelect) {
					...
					_reactivemaps.AppbaseSensorHelper.URLParams.update(this.props.componentId, this.setURLParam(_obj.value), this.props.URLParams);
					_reactivemaps.AppbaseSensorHelper.selectedSensor.set(_obj, true);
				} else {
					(function () {
						var records = [];
						if (_this3.defaultSelected) {
							_this3.defaultSelected.forEach(function (item) {
								records = records.concat(_this3.props.data.filter(function (record) {
									return item.indexOf(record.label) > -1;
								}));
							});
						} else if (_this3.defaultSelected === null) {
							records = null;
						}
						_this3.setState({
							selected: records
