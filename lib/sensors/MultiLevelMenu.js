	...
	_createClass(MultiLevelMenu, [{
		...
		value: function secondLevelAggCustomQuery() {
			...
			if (this.secondLevelMenu) {
				...
				Object.keys(this.secondLevelMenu).forEach(function (item) {
					_this6.secondLevelMenu[item][_this6.props.dataField[1]].buckets.forEach(function (nestedItem) {
		...
		value: function getAggFilterQuery(items) {
			...
			items.forEach(function (item, index) {
				var obj = {
					term: _defineProperty({}, _this7.props.dataField[index], item)
				};
		...
		value: function setData(data) {
			...
			Object.keys(data.aggregations).forEach(function (level1) {
				...
				var finalMenu = data.aggregations[level1][_this8.props.dataField[2]].buckets.map(function (item) {
					return item.key;
				});
...
MultiLevelMenu.propTypes = {
	...
	dataField: _react2.default.PropTypes.array.isRequired,
