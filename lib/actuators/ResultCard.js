	...
	_createClass(ResultCard, [{
		...
		value: function componentWillUpdate() {
			...
			setTimeout(function () {
				...
				if (_this2.showPagination !== _this2.props.showPagination) {
					_this2.showPagination = _this2.props.showPagination;
				...
				if (_this2.showPagination && _this2.paginationAtVal !== _this2.props.paginationAt) {
		...
		value: function listenLoadingChannel(channelObj) {
			...
			this.loadListener = channelObj.emitter.addListener(channelObj.channelId + "-query", function (res) {
				if (res.appliedQuery) {
					var showInitialLoader = _this4.showPagination || !res.appliedQuery.body || !res.appliedQuery.body.from;
					_this4.setState({
			...
			function setScroll(node) {
				...
				if (node) {
					node.addEventListener("scroll", function () {
						if (!_this7.showPagination && $(node).scrollTop() + $(node).innerHeight() >= node.scrollHeight && _this7.state.resultStats.total > _this7.state.currentData.length && !_this7.state.queryStart) {
							_this7.nextPage();
						}
					});
				}
...
ResultCard.types = {
	...
	showPagination: _reactivemaps.TYPES.BOOLEAN,
